        ...
        for event in pygame.event.get():
            ...
            if event.type in range(field.progress_enc_start, field.progress_enc_end):
                ...
                if node.encrypted:
                    ...
                    field.count += 500
                    if field.alarm_time is True:
                        field.alarm_time_progres += 3
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.USEREVENT:
                if event.button == lightgame_button:
                    ...
                    play_game({'rows': 9, 'cols': 10, 'mode_coeff': 1.5, 'HP': 30})
                elif event.button == mediumgame_button:
                    play_game({'rows': 9, 'cols': 10, 'mode_coeff': 1.0, 'HP': 15})
